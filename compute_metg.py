...
def compute_anomaly(recette, window_size, hidden_size, columns_to_drop, config):
    try:
        ...
        print(f"Chargement du modèle METG pour la recette {recette}...")
        
        # Load model parameters
        model_params_file = f"output//models//{recette}_metg_params.yaml"
        if not os.path.exists(model_params_file):
            raise FileNotFoundError(f"Model parameters file not found: {model_params_file}")
        
        with open(model_params_file, "r", encoding="utf-8") as f:
            model_params = yaml.safe_load(f)
        
        # Create METG model with saved parameters
        ...
        # Set loss weights
        model.lambda1 = model_params['lambda1']
        model.lambda2 = model_params['lambda2']
        
        # Load model state
        model_state_file = f"output//models//{recette}_metg_model.pth"
        ...
        scalers_path = f"output//models//{recette}_metg_scalers.pkl"
        ...
        for idx, fname in enumerate(all_files):
            ...
            # Create windows for METG (3D format)
            ...
            for batch_start in range(0, len(windows_tensor), batch_size):
                ...
                batch_windows = windows_tensor[batch_start:batch_end].to(device)
                
                with torch.no_grad():
                    # Compute anomaly scores using METG
                    anomaly_scores = model.compute_anomaly_score(batch_windows)
                    # anomaly_scores shape: (batch_size, window_size)
                    
                    # Aggregate scores per window (take max over time dimension)
                    window_scores = anomaly_scores.max(dim=1)[0]  # Shape: (batch_size,)
        ...
        if all_results:
            ...
            final_results.to_excel(os.path.join(output_dir, f"{recette}_metg_anomaly_results.xlsx"), index=False)
            print(f"Scores d'anomalies METG enregistrés à '{recette}_metg_anomaly_results.xlsx'")
        else:
            print("Aucun fichier valide.")
    
        print("Tous les fichiers ont été traités avec METG.")

        return True
    except Exception as e:
        print(f"Erreur computing METG {e}")
        import traceback
        traceback.print_exc()
        sys.exit(1)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        for recette in recette_entree:
            computed = compute_anomaly(recette, window_size, hidden_size, columns_to_drop, config)
        ...
        print("Utilisation : python compute_metg.py <recette>")
        sys.exit(1)